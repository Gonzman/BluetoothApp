...
extension Bluetooth: CBCentralManagerDelegate, CBPeripheralDelegate{
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        if !peripherals.contains(peripheral){
            ...
            self.peripheralNames.append(peripheral.name ?? "Nicht benanntes Gerät: \(peripheral.identifier)")
        }
    }
    ...
    func getPeripheralName(peripheral: CBPeripheral) -> String {
        return peripheral.name ?? "Nicht benanntes Gerät: \(peripheral.identifier)"
    }
